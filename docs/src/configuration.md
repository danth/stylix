...
To start theming, you need to set a wallpaper with a [constructor](lib/wallpaper-constructors.md).
...
The image argument accepts derivations as well as paths, so you can fetch an image
...
You can force a light or dark scheme using the polarity argument:
...
This constructor also accepts other files and formats supported by
[`mkSchemeAttrs`](https://github.com/SenchoPens/base16.nix/blob/main/DOCUMENTATION.md#mkschemeattrs).

### Overriding
For convenience, it is possible to override the certain colors in the base16 scheme generated by the wallpaper constructors by using the optional
override argument. this can be an attrset containing the colors to be overridden, or a base16 yml file that is also used int the mkStaticFill constructor.
