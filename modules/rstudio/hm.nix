...
{
  options.stylix.targets.rstudio.enable = config.lib.stylix.mkEnableTarget "RStudio" true;

  config = lib.mkIf (config.stylix.enable && config.stylix.targets.rstudio.enable) {
    ...
    home.activation.rstudioThemeSelect =
      let
        file = builtins.toFile "rstudio-prefs.json" (builtins.toJSON { editor_theme = name; });
