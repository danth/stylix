...
mkTarget {
  ...
  configElements = [
    (
      { cfg }:
      {
        programs.waybar.style = lib.optionalString cfg.addCss ''
    ...
    (
      ...
      {
        stylix.targets.waybar.background = lib.optionalString cfg.addCss "alpha(@base00, ${builtins.toString opacity.desktop})";
      }
    )
    (
      { cfg, fonts }:
    ...
    (
      ...
      in
      {
        stylix.targets.waybar.background = "@base00";
        programs.waybar.style =
