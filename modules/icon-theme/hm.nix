{
  ...
  recolorScript = with cfg.recolor; ''
    ...
    ${lib.optionalString (accentSaturation != null) "--accent-saturation ${accentSaturation}"} \
      ...
      ${
      if cfg.recolor.mode == "monochrome"
      then "--monochrome '${builtins.concatStringsSep "," cfg.recolor.colors}'"
      else "--palette ''${builtins.concatStringsSep "," cfg.recolor.colors}''"
    }
  '';
in {
          ...
          package =
            ...
            (oldAttrs: rec {
              propagatedBuildInputs =
                if cfg.recolor.enableForParentThemes
                then
                  builtins.map
                  (parentIconTheme:
                    parentIconTheme.overrideAttrs (parentIconThemeOldAttrs: rec {
