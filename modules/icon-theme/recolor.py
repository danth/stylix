...
def css_to_hex(text: str) -> str:
    ...
    text = re.sub(
        r"rgba\((\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*,\s*([\d.]+)\)", expand_css_rgba, text
    )
...
def apply_modifications(
    old_color, new_color_hsl: Tuple[float, float, float], args
...
def apply_monotones_to_vec(
    text: str, colors: Set[str], hsl: Tuple[float, float, float], args
) -> str:
    ...
    if s == 0:
        ...
        for color in colors:
            ...
            monochrome = rgb_to_hex(
                hsl_to_rgb(apply_modifications(color, (h, s, l), args))
            )
...
def apply_palette_to_vec(
    text: str, colors: Set[str], new_colors: Dict[str, LabColor], args
) -> str:
    ...
    for color in colors:
        ...
        new_color = rgb_to_hex(
            hsl_to_rgb(apply_modifications(color, hex_to_hsl(new_color), args))
        )
    ...
    if s == 0:
        ...
        for x in range(width):
            for y in range(height):
                ...
                new_color = hsl_to_rgb(
                    apply_modifications(rgb_to_hex((r, g, b)), (h, s, l), args)
                )
...
def extract_colors(
    ...
    if ext == ".svg":
        ...
        colors = [
            "#{:02X}{:02X}{:02X}".format(colors[i], colors[i + 1], colors[i + 2])
...
def add_backdrop(
    ...
    for path in tqdm(svg_paths, desc="Changing svgs  ", unit="file"):
        ...
        backdrop = (
            ...
            + str(width - 2 * padding)
            + '" height="'
            + str(height - 2 * padding)
            + '" rx="'
            + str(rounding * (width / 2))
            + '" ry="'
            + str(rounding * (height / 2))
            + '"/>'
        )
